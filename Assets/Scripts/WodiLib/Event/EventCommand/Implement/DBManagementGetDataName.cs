...
// File Name    : DBManagementGetDataName.cs
//
...
namespace WodiLib.Event.EventCommand
{
    /// <inheritdoc />
    /// <summary>
    /// イベントコマンド・DB操作（データ名取得）
    /// </summary>
    [Serializable]
    public class DBManagementGetDataName : DBManagementOutputBase
    {
            ...
            EventCommandSentenceResolveDesc desc)
        {
            ...
            var paramData = resolver.GetNumericVariableAddressStringIfVariableAddress(
                DBDataId.ToInt(), type, desc);
            var targetData = resolver.GetDatabaseDataName(DBKind, typeId, DBDataId.ToInt()).Item2;
